...
app.layout = html.Div([
    ...
    dcc.Loading(
        ...
        children=dcc.Graph(id='amplitude-plot', config={
            'modeBarButtonsToAdd': ['drawline', 'eraseshape', 'zoom', 'zoomIn', 'zoomOut', 'resetScale2d'],
            'displaylogo': False
        })
    ),
    html.Div([
...
def update_amplitude_plot(contents, n_clicks, figure):
    ...
    if ctx.triggered and ctx.triggered[0]['prop_id'].split('.')[0] == 'confirm-button':
        if n_clicks > 0 and figure and 'layout' in figure and 'shapes' in figure['layout']:
            for shape in figure['layout']['shapes']:
                shape['line']['color'] = 'rgb(249,249,249)'
                shape['line']['width'] = 1
...
def update_time_inputs(relayoutData):
